import React, { Component } from 'react';

class Contents33 extends Component {
  render() {
    return <section className="fdb-block py-0">
  <div className="container pt-5 mt-5" style={{backgroundImage: 'url(imgs/shapes/9.svg)'}}>
    <div className="row text-left pt-5">
      <div className="col-12 col-md-8 col-lg-6">
        <div className="fdb-box fdb-touch rounded-bottom-0">
          <h2><strong>Design Blocks</strong></h2>
          <p className="lead">Far far away, behind the word mountains, far from the countries Vokalia and Consonantia, there live the blind texts. Separated they live in Bookmarksgrove right at the coast of the Semantics</p>
          <p className="lead mt-4"><a href="https://www.froala.com">Read More</a></p>
        </div>
      </div>
    </div>
  </div>
</section>

  }
}

export default Contents33
